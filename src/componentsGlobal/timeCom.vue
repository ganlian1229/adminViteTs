<template>
    <div class="time-box">
        <span class="year">{{ yearTime }}</span>
        <span class="week">{{ weekTime }}</span>
        <span class="time">{{ time }}</span>
        <p>农历{{ farmTime.IMonthCn }}{{ farmTime.IDayCn }}</p>
    </div>
</template>
...
function startTime() {
    ...
    let mf = newDate.getMinutes() < 10 ? '0' + newDate.getMinutes() : newDate.getMinutes();
    let ss = newDate.getSeconds() < 10 ? '0' + newDate.getSeconds() : newDate.getSeconds();
    yearTime.value = yy + '.' + mm + '.' + dd;
    time.value = hh + ':' + mf + ':' + ss;
    farmTime.value = calendar.solar2lunar();
    //当前星期
